		...
		for chr_arm, mrkr_and_pos in chr_arm_dict.items(): 
			...
			for index in range(0,len(marker_list)):
				...
				if first_recom == 1 and (marker == marker_list[stor_idx]):
					pass # With the list, keep going, this is already added to the list.
				...
				elif first_recom == 0 and \
					((marker == marker_list[stor_idx]) and ((index + 1) == len(marker_list))):
					recom_count += 1
					...
					((marker != marker_list[stor_idx]) and ((index + 1) == len(marker_list))):
					recom_count += 2
					event_size_1 = abs(recom_frag_dict[ "positions"][0] - recom_frag_dict[ "positions"][-1])
					event_1 = ",".join( [chr_name, str(recom_frag_dict[ "positions"][0]), str(event_size_1)] )
					print( prog_name + "," + event_1, file = output_file)
					prog_data[ "recoms" ].append( event_1 )
					if arm_side == "left":
						event_size_2 = posit_list[ index ]
					else: 
						event_size_2 = abs(posit_list[ index ] - chr_lengths[ chr_name ])
					event_2 = ",".join( [chr_name, str(recom_frag_dict[ "positions"][0]), str(event_size_2)] )
					print( prog_name + "," + event_2, file = output_file)
					prog_data[ "recoms" ].append( event_2 )
