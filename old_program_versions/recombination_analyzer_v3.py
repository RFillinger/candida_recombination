...
def recombination_analysis( f2_file_name, output_file_name, centromeres, chr_lengths, a = "1", b = "2", h = "n", centromere_rules = 1):
	...
	for progeny in children: 
		...
		recombs[ progeny[0] ] = { "marker_tally":{}, "recoms": [] }
		prog_data = recombs[ progeny[0] ]

		# Initiate the storage variables, and give them a value that is easy to identify as empty.
		stor_chr = "empty"  
		stor_mkr = "empty"
		stor_pos = -1
		...
		for i, marker in enumerate(progeny):
			...
			if (marker == progeny[0]): # If it's the parent column or a missing marker, skip it. 
				continue 

			"""
			Logic flow in this next section looks unnecessary, but I need it if I want to skip the missing markers.
			Keep the others and analyze after that.
			"""
			if marker == "-": 
				# Skip this one, but note its presence so I can have some error checking in my plots. 
				prog_data[ "marker_tally" ] [ ",".join([chromosomes[i], chr_pos[i]] ) ] = "-" 
				continue
			elif marker == a:
				prog_data[ "marker_tally" ] [ ",".join([chromosomes[i], chr_pos[i]] ) ] = a
			elif marker == b:
				prog_data[ "marker_tally" ] [ ",".join([chromosomes[i], chr_pos[i]] ) ] = b
			elif marker == h: 
				prog_data[ "marker_tally" ] [ ",".join([chromosomes[i], chr_pos[i]] ) ] = h
			else: 
				print( "There is a problem with this data input: Marker content" )
				print( "Marker position: ", [progeny[0], chromosomes[i], chr_pos[i]] )
				quit()

			if (stor_chr == "empty") or (stor_mkr == "empty"):
				# Initiate the first variables; No recombination has occurred.
				stor_chr = chromosomes[i]  
				stor_mkr = marker
				stor_pos = int(chr_pos[i])
				continue

			if centromere_rules: 

				left_side_centro = centromeres[ stor_chr ][0]
				rite_side_centro = centromeres[ stor_chr ][1]
				stor_left_o_centro = stor_pos <= left_side_centro
				stor_rite_o_centro = stor_pos >= rite_side_centro
				mkr_left_o_centro = int( chr_pos[i] ) <= left_side_centro
				mkr_rite_o_centro = int( chr_pos[i] ) >= rite_side_centro

				same_mkr = progeny[i-1] == marker # Boolean to check if there is potential for recombination
				same_chr = chromosomes[i-1] == chromosomes[i] # Boolean to check if we're on the same chromosome
				
				try: 
					end_chr = chromosomes[i] != chromosomes[i+1]
				except IndexError: # The very last chromosome will trigger this.
					end_chr = 1
		

				if end_chr and stor_rite_o_centro:
					event_size = chr_lengths[chromosomes[i]] - int( stor_pos )
					event = ",".join( [chromosomes[i], str( stor_pos ), str(event_size)] )
					prog_data[ "recoms" ].append( event )
					recom_count += 1
					print( progeny[0] +","+ event , file = output_file )

				elif same_mkr and same_chr:
					continue
				...
				elif not same_mkr and same_chr: 

					if stor_left_o_centro: 

						if mkr_left_o_centro: 
							event_size = int(stor_pos) - int(chr_pos[i]) 
							event = ",".join( [chromosomes[i], str( chr_pos[i] ), str(event_size)] ) # Using this method enables use of the first OR second bins for recombination
							prog_data[ "recoms" ].append( event )
							recom_count += 1
							print( progeny[0] +","+ event , file = output_file )
							# Reassign the variables for the next round

						elif mkr_rite_o_centro: # We're treating this as if it was impossible
							stor_chr = chromosomes[i] 
							stor_mkr = marker
							stor_pos = int(chr_pos[i])
							continue 
							...
							event_size = stor_pos - left_side_centro 
							event = ",".join( [chromosomes[i], str( left_side_centro ), str(event_size)] )
							prog_data[ "recoms" ].append( event )
							recom_count += 1
							print( progeny[0] +","+ event , file = output_file )

					elif stor_rite_o_centro:

						event_size = int(chr_pos[i]) - stor_pos
						event = ",".join( [chromosomes[i], str( stor_pos ), str(event_size)] )
						prog_data[ "recoms" ].append( event )
						print( progeny[0] +","+ event , file = output_file )
						...
						if mkr_rite_o_centro:
							event_size = left_side_centro - stor_pos
							event = ",".join( [chromosomes[i], str( left_side_centro ), str(event_size)] )
							prog_data[ "recoms" ].append( event )
							recom_count += 1
							print( progeny[0] +","+ event , file = output_file )

						else: # Both are in the centromere
							# I have no idea what to do here, so I'll just convert it to a different situation! 
							continue
				...
				if stor_mkr == marker:
					...
					Literally have to do nothing, just carry on."""
					pass
				elif (stor_mkr != marker) and (stor_chr != chromosomes[i]): 
					"""If the chromosome has changed, count the last changed section as a recombination and reset the storage values. 
					This scenario should be caught earlier, but I was entering the "else" statement without this; recombination exception. """
					stor_chr = chromosomes[i] 
					stor_mkr = marker
					stor_pos = int(chr_pos[i])
				elif (stor_mkr != marker) and (stor_chr == chromosomes[i]): 
					"""The current marker has changed from the original marker and they're on the same chromosome!
					Compute the size and store the event."""
					event_size = int(chr_pos[i]) - int(stor_pos)
					event = ",".join( [chromosomes[i], str( chr_pos[i] ), str(event_size)] ) # Using this method enables use of the first OR second bins for recombination
					prog_data[ "recoms" ].append( event )
					recom_count += 1
					print( progeny[0] +","+ event , file = output_file )
					...
					print( "There is a problem with this data input: Recombination exception" )
					print( "Marker position: ", [progeny[0], chromosomes[i], chr_pos[i]] )
					quit()
...
def main(create_tester = 0):
	...
	if test_bool: 
		...
		dir_path, file_name = parse_path( path_and_name )
		test_creator( path_and_name, 42, 1000, 2)
