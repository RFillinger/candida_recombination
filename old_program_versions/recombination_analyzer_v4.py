...
def recombination_analysis( f2_file_name, output_file_name, centromeres, chr_lengths, a = "1", b = "2", h = "n", centromere_rules = 1):
	...
	for chro, centros in centromeres.items(): 
		...
		left_rite_indies[ left_arm_key ] = {"chro": chro, "chr_pos":[], "markers": []}
		...
		for i, chr_strings in enumerate(chromosomes):
			if chro == chr_strings: 
				if int(chr_pos[i]) <= centromere_left_bound:
					left_rite_indies[ left_arm_key ]["chr_pos"].insert( 0,int(chr_pos[i]) ) # These will be listed to count recombinations away from the centromere
				if int(chr_pos[i]) >= centromere_rite_bound:
	...
	for progeny in children: 
		...
		for chromosome_arms, index_list in left_rite_indies.items(): 
			recom_1_bool = 1
			stor_pos_i = index_list[0]
			event_size = 0
			index_count = 0
			for i in index_list: 

				# print( index_count == len(index_list)-1)
				print()
				print( chromosome_arms )
				print( i, stor_pos_i )
				print( chr_pos[i], chr_pos[stor_pos_i])


				prog_data[ "marker_tally" ][ ",".join([chromosomes[i], chr_pos[i]] ) ] = progeny[i]
				if progeny[i] == "-":  
					continue

				if (progeny[i] == progeny[stor_pos_i]) and recom_1_bool:
					print( "equal")
					stor_pos_i = i

				elif (progeny[i] != progeny[stor_pos_i]): 
					
					recom_count += 1 

					recom_1_bool = 0

					if index_count == len(index_list)-1: # this doesn't catch the ends of the chromosomes
						print( "IN!" )
						if recom_1_bool:
							if chromosome_arms[-4:] == "left":
								event_size = int(chr_pos[i]) 
							else: 
								event_size = abs(int(chr_pos[i]) - chr_lengths[chromosomes[i]])
						else: 
							if chromosome_arms[-4:] == "left":
								event_size += int(chr_pos[i])
							else: 
					...
					event = ",".join( [chromosomes[i], chr_pos[i], str(event_size)] )
					...
					if chr_pos[i-1] == chr_pos[0]: 
						continue

					event_size += abs( int(chr_pos[i]) - int(chr_pos[i-1]) )
					# event_size += abs( int(chr_pos[i+1]) - int(chr_pos[i]) )
